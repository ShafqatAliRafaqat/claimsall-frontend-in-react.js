...
class OrganizationsView extends React.Component {
        ...
        fetchOrganizationTypes()
            .then(organizationTypeFilters => {
                ...
                organizationTypeFilters.forEach(type => {
                    ...
                    if (type.code === 'company') {
                        _typeFilters.organization_type_id.push(type.id + '');
                    }
                });

                this.setState({organizationTypeFilters: filters, filters: _typeFilters}, () => {
    ...
    showDeleteConfirm = (id) => {
        ...
        let _this = this;
        confirm({
            title: 'Are you sure you want to delete selected organization(s) ?',
            content: '',
            okText: 'Yes',
            okType: 'danger',
            cancelText: 'No',
        ...
        return [
            {
                ...
                render: (text, record) => (
                    <a className="ant-dropdown-link hover" onClick={() => this.props.history.push({
                        pathname: '/organizations/users/all',
                        state: {organization_id: record.id, organization: record}
                    })}>
                        {text}
                    </a>
                ),
            },
            {
                ...
                filters: this.state.organizationTypeFilters,
                filteredValue: this.state.filters.organization_type_id || []
            },
            {
                title: 'UAN#',
        ...
        if (this.state.selectedRowKeys.indexOf(record.id) > -1) {
            return (
                <div>
                    ...
                    <Link to={{pathname: `/organizations/edit`, state: {...this.props.location.state, id: record.id}}}>
    ...
    deleteAll = () => {
        if (false && this.state.selectedRowKeys && this.state.selectedRowKeys.length > 0) {
            return (
            ...
            return (
                <Button className='shadow' style={{fontWeight: 'bold'}}
                        onClick={() => this.props.history.push({pathname: '/organizations/add'})}>
                    <Icon type='plus' style={{fontSize: 16}}/>Add Organization
                </Button>
            );
        }
    };
    ...
    onInputChange = (e) => {
        ...
        if (searchVal && searchVal.length) {
            ...
            columns.forEach(column => {
                if (column.dataIndex && ["employee", "organization_type_name", "created_at"].indexOf(column.dataIndex) < 0) {
            ...
            <div>
                <Row type="flex" justify="space-around" align="middle" gutter={32}>
                    <Col span={23} offset={1}>
                        <h3 className='text-left' style={{fontSize: '36px', fontWeight: 900}}>Organizations</h3>
                    </Col>
                </Row>
                            ...
                            <Col span={15} offset={6}>
                                <Row>
                                    <Col span={10} offset={4}>
                                        <ReportButton headers={headers} filename={"organizations.csv"}
